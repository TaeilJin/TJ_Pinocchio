...
def genCOMConstraints(rotation = Id, normals = [z for _ in range(len(limbNames))]):
    # : normals, rotation  은 z축, identity 이다.
    res =[]
    for idx in range(len(limbNames)):
        res += [com_in_limb_effector_frame_constraint(default_transform_from_pos_normal_(rotation, zero3, normals[idx]),idx)]
    return res
